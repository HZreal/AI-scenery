...
func newOpenAIProvider(settings Settings) (Provider, error) {
	if strings.TrimSpace(settings.OpenAIAPIKey) == "" {
		return nil, NewError("missing_api_key", "AI_SCENERY_PROVIDER=openai 时必须设置 OPENAI_API_KEY", 500)
	}
...
func (p *openAIProvider) Generate(ctx context.Context, request Request) (Result, error) {
	...
	if err != nil {
		return Result{}, NewError("provider_api_error", "OpenAI API 调用失败: "+err.Error(), 502)
	}
	return p.resultFrom(response, request.Mode)
}

func (p *openAIProvider) Stream(ctx context.Context, request Request, emit func(string) error) (Result, error) {
	stream := p.client.Responses.NewStreaming(ctx, openAIParams(Request{Messages: request.Messages, Mode: ModeText}, p.model))
	defer stream.Close()
	...
	if err := stream.Err(); err != nil {
		return Result{}, NewError("provider_stream_error", "OpenAI 流式调用失败: "+err.Error(), 502)
	}
	if completed == nil {
		return Result{}, NewError("provider_response_invalid", "OpenAI 未返回完成事件", 502)
	}
	return p.resultFrom(completed, ModeText)
}

func (p *openAIProvider) resultFrom(response *responses.Response, mode Mode) (Result, error) {
	text := response.OutputText()
	if text == "" {
		return Result{}, NewError("provider_response_invalid", "OpenAI 响应中没有文本内容", 502)
	}
	inputTokens := int32(response.Usage.InputTokens)
	outputTokens := int32(response.Usage.OutputTokens)
	result := Result{Value: text, Model: p.model, InputTokens: &inputTokens, OutputTokens: &outputTokens}
	if mode == ModeJSON {
		var value map[string]any
		if err := json.Unmarshal([]byte(text), &value); err != nil {
			return Result{}, NewError("provider_json_invalid", "OpenAI 未返回有效 JSON 对象", 502)
		}
		result.Value = value
	}
	return result, nil
...
func openAIParams(request Request, model string) responses.ResponseNewParams {
